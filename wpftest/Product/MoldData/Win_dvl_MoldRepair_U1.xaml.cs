...
{
    /// <summary>
    /// Win_dvl_MoldRepair_U.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class Win_dvl_MoldRepair_U1 : UserControl
    {
        #region 변수선언 및 로드
        ...
        private void ControlVisibleAndEnable_AU()
        {
            ...
            dgdMoldRepair.IsHitTestVisible = false;
        ...
        private void ControlVisibleAndEnable_SC()
        {
            ...
            dgdMoldRepair.IsHitTestVisible = true;
            ...
            if (ExpExc.DialogResult.HasValue)
            {
                if (ExpExc.choice.Equals(dgdMoldRepair.Name))
                {
                    ...
                    if (ExpExc.Check.Equals("Y"))
                        dt = Lib.Instance.DataGridToDTinHidden(dgdMoldRepair);
                    else
                        dt = Lib.Instance.DataGirdToDataTable(dgdMoldRepair);
                ...
                else if (ExpExc.choice.Equals(dgdMoldRepairSub.Name))
                {
                    //MessageBox.Show("소분류");
                    if (ExpExc.Check.Equals("Y"))
                        dt = Lib.Instance.DataGridToDTinHidden(dgdMoldRepairSub);
                    else
                        dt = Lib.Instance.DataGirdToDataTable(dgdMoldRepairSub);
        ...
        private void re_Search(int selectIndex)
        {
            ...
            //this.DataContext = null;    // 20190610 추가
            FillGrid();
            ...
            if (dgdMoldRepair.Items.Count > 0)
            {
                ...
                {
                    dgdMoldRepair.SelectedIndex = Lib.Instance.reTrunIndex(dgdMoldRepair, lstCompareValue[0]);
                }
                else
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in drc)
                        {
                            var MoldRepair_DTO = new Win_dvl_MoldRepair_U_CodeView()
                            {
                                ...
                                repairTime = dr["repairTime"].ToString(),
                            ...
                            MoldRepair_DTO.repairdate = Lib.Instance.StrDateTimeBar(MoldRepair_DTO.repairdate);

                            if (MoldRepair_DTO.Weight.Contains("."))
                            {
                                if (int.Parse(MoldRepair_DTO.Weight.Substring(MoldRepair_DTO.Weight.IndexOf(".") + 1)) > 0)
                                {
                                    MoldRepair_DTO.Weight = Lib.Instance.returnNumStringTwo(MoldRepair_DTO.Weight);
                                }
                                else
                                {
                                    MoldRepair_DTO.Weight = Lib.Instance.returnNumStringZero(MoldRepair_DTO.Weight);
                                }
                            }
                            else
                            {
        ...
        private void dgdMoldRepair_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (winMoldRepair != null)
            {
                ...
                txtMold.Tag = winMoldRepair.MoldID;
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in drc)
                        {
                            ...
                            dgdSub_DTO.partcnt = Lib.Instance.returnNumStringZero(dgdSub_DTO.partcnt);
                            dgdSub_DTO.partprice = Lib.Instance.returnNumStringZero(dgdSub_DTO.partprice);
        ...
        private bool InsertData()
        {
            ...
            if (CheckData())
            {
                try
                {
                    ...
                    {
                        DataGridRow dgr = Lib.Instance.GetRow(i, dgdMoldRepairSub);
                        ...
                        sqlParameter.Add("partcnt", winRepairSub.partcnt.Replace(",", ""));
                        sqlParameter.Add("partprice", winRepairSub.partprice.Replace(",", ""));
